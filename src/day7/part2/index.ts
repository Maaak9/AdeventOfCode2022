...
class FileSystemNode {
  ...
  constructor({ name, parent, children, size, dirSize }){
    this.name = name;
    this.parent = parent;
...
const buildFileSystem = (instructions: string[]) => {
  const rootNode = new FileSystemNode({ name: "root", children: [], parent: undefined, size: undefined });
  ...
  instructions.forEach((i) => {
    ...
    if (instruction.match(/cd \.\./)) {
      ...
      if (childNode[0]) {
        ...
        fileSystem.addChildNode(new FileSystemNode({ name: folderName, parent: fileSystem.currentNode, children: [], size: undefined }));
      }
    }
...
const currentUsedSize = fileSystem.rootNode.dirSize;
const freeSpace = totalDiskSize - currentUsedSize;
const minimumSpaceNeeded = updateSize - freeSpace

let smallestDirNeeded = 100000000;

dirSizes.sort().forEach((dirSize) => {
